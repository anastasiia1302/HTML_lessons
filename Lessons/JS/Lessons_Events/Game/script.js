...
for (var i = 0; i < 9; i++) {
	...
	innerElement.setAttribute('y', parseInt(i / 3).toString());//у - это номер строки, х - номер столбца
...
function tableClick() {
	if (this.innerText == '') {
		...
		gamer1 = !gamer1;
		player.innerText = gamer1 ? 'Суйчас ходит Х' : 'Сейчас ходит О';
...
//Функция сброса парамтров игры
function reset() {
	gamer1 = true;// Показывает, какой игрок с ейчас ходит
	gameTable = [[null, null, null], [null, null, null], [null, null, null]];// Матрица игры
	nullCount = 9; // Количество оставшихся ходов
	winner = null;
